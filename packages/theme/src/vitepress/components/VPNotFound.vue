...
<template>
  <div class="vt-doc">
    ...
    <p>
      ...
      }}<span class="not-found-path">{{ route.path }}</span>{{ config.i18n?.deadLink?.after ?? '' }}<br><span
        v-if="config.editLink?.repo"
      >{{ config.i18n?.deadLinkReport?.before ?? 'Please '
      }}<a
        :href="`https://github.com/${config.editLink?.repo}`"
        target="_blank"
      >{{ config.i18n?.deadLinkReport?.link ?? 'let us know' }}</a>{{ config.i18n?.deadLinkReport?.after ?? ' so we can fix it.' }}</span>
    </p>
  </div>
</template>
