...
<template>
  <div ref="container" class="VPContentDocOutline">
    ...
    <nav aria-labelledby="doc-outline-aria-label">
      ...
      <ul class="root">
        <li v-for="{ text, link, children, hidden } in filteredHeaders" v-show="!hidden" :key="link">
          <a class="outline-link" :href="link" @click="handleClick">{{
            text
          }}</a>
          <ul v-if="children && frontmatter.outline === 'deep'">
            <li
              v-for="{ text: child_text, link: child_link, hidden: child_hidden } in children" v-show="!child_hidden"
              :key="child_link"
            >
              <a class="outline-link nested" :href="child_link" @click="handleClick">{{ child_text }}</a>
            </li>
          </ul>
        </li>
      </ul>
